...
static const char * OUI_PATHS[]
	= {"./airodump-ng-oui.txt",
	   "/home/kali/Desktop/wiresharkOUI.txt",	
	...
	if (h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_REQ && st_cur != NULL)
	{
		...
		while (p < h80211 + caplen)
		{
			...
			if (p[0] == 0xDD)
			{
				...
				unsigned char array_vendor_specific[vendor_specific_length];
				...
				for(i = 0; i < 4; i++)
				{
					...
					array_vendor_specific[i] = p[2 + i];
					array_v[pos] = p[2 + i];
					pos++;

					printf("%X ", p[2 + i]);

					if(i == vendor_specific_length -1){ printf("\n");}
				}

				if(vendor_specific_count == 0)
				{
					vendor_1_specific_flag = 1;
					u64t_vendor_specific_1 = t1ha0(array_vendor_specific, sizeof(array_vendor_specific), (uint64_t)seed);
				...
				}else if(vendor_specific_count == 1)
				{
					...
					u64t_vendor_specific_2 = t1ha0(array_vendor_specific, sizeof(array_vendor_specific), (uint64_t)seed);
				...
				}else if(vendor_specific_count == 2)
				{
					...
					u64t_vendor_specific_3 = t1ha0(array_vendor_specific, sizeof(array_vendor_specific), (uint64_t)seed);
				...
				}else
				{
					...
					u64t_vendor_specific_4 = t1ha0(array_vendor_specific, sizeof(array_vendor_specific), (uint64_t)seed);
		...
		/*
		if(ssid_flag == 1){ printf("SSID ID exists!\n");}
		if(supported_rates_flag == 1){ printf("Supported Rates exists!\n");}
		if(extended_supported_rates_flag == 1){ printf("Extended Supported Rates exists!\n");}
		if(DS_param_flag == 1){ printf("DS Param exists!\n");}
		if(HT_capabilities_flag == 1){ printf("HT Capabilities flag exists!\n");}
		if(extended_capabilities_flag == 1){ printf("Extended Capabilities exists!\n");}
		if(VHT_capabilities_flag == 1){ printf("VHT Capabilities exists!\n");}
		if(vendor_specific_flag == 1){ printf("Vendor Specific exists!\n");}
		*/
		
		char * ssid;
		ssid = (const unsigned char *)st_cur->probes[st_cur->probe_index];
		...
		printf("pos: %i\n", pos);
		printf("Array: \n");
		for(int s = 0; s < sizeof(array_v); s++ )
		{
			printf("%2X ", array_v[s]);
		}
		printf("\n");
		*/
		...
		/*
		printf("Size of New array: %i\n", sizeof(new_array));
		printf("pos: %i\n", pos);
		printf("New Array: \n");
		for(int v = 0; v < sizeof(new_array); v++ )
		{
			printf("%2X ", new_array[v]);
		}
		printf("\n");
		*/
		...
		if(vendor_1_specific_flag == 0){ strcpy(vendor_1, "NaN");}else{ strcat(IE_utilized, "221 ");}
		if(vendor_2_specific_flag == 0){ strcpy(vendor_2, "NaN");}
		if(vendor_3_specific_flag == 0){ strcpy(vendor_3, "NaN");}
		if(vendor_4_specific_flag == 0){ strcpy(vendor_4, "NaN");}

		printf("--------------Information Elements: Footprints-------------\n");
		printf("Supported Rates: %s\n", supp_rates);
		printf("Extended Supported Rates: %s\n", ext_supp_rates);
		printf("DS Parameter Set: %s\n", DS_param);
		printf("HT Capabilities: %s\n", HT_cap);
		printf("Extended Capabilities: %s\n", ext_cap);
		printf("VHT Capabilities: %s\n", VHT_cap);
		printf("RM Enabled Capabilities: %s\n", RM_enabled_cap);
		printf("Interworking: %s\n", interworking);
		printf("Supported Operating Classes: %s\n", supp_operating_classes);
		...
		printf("Information Elements utilizados: %s\n", IE_utilized);
		...
		printf("Probe Request | Source: %s | Destination: %s | SSID: %s | SEQ: %i | Channel: %i | Power: %i dB | Best Power: %i dB | Manufacturer: %s \n", mac_address, bssid_addr, ssid, seq, channel, power, best_power, manuf);
		

		putInToDB(mac_address , footprint, IE_utilized, supp_rates, ext_supp_rates, DS_param, HT_cap, ext_cap, VHT_cap, RM_enabled_cap, interworking, supp_operating_classes, vendor_1, vendor_2, vendor_3, vendor_4, received_power, manuf, db);
...
skip_probe:
		...
		|| h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_RESP)
	{
		...
		//printf("Probe Responde or Beacon found from MAC Address: %s \n", bssid_addr);
	}
...
void putInToDB(char* mac_address, char* footprint, char* IE_utilized, char* supp_rates, char* ext_supp_rates, char* DS_param, char* HT_cap, char* ext_cap, char* VHT_cap, char* RM_enabled_cap, char* interworking, char* supp_operating_classes, char* vendor_1, char* vendor_2, char* vendor_3, char* vendor_4, char* received_power, char* manuf, sqlite3 *db){
	...
	/*
	// tabela de 15 minutos

    sqlstatment = "Select * from Cellphone_Records_15 where Cast (( JulianDay('now') - JulianDay(Last_Time_Found) ) * 24 * 60 * 60 As Integer)  < 15*60  and ID ='" ;
    strC = concat(sqlstatment , concat(mac_address , "' ;"));
	#ifdef DEBUG
    	printf(strC);
	#endif

    //result = sqlite3_exec(db, strC, callbackSelect, NULL, &zErrMsg);
    ...
    if (sqlite3_step(selectstmt) == SQLITE_ROW)
       {
          // record found
          sqlite3_finalize(selectstmt);
			sqlstatment = "UPDATE Cellphone_Records_15 set Last_Time_Found= CASE WHEN Cast(( JulianDay('now') - JulianDay(First_Record) ) * 24 * 60 * 60 As Integer)   > 300  THEN CURRENT_TIMESTAMP ELSE Last_Time_Found  END ,  contador=contador+1 WHERE Tecnologia='Wi-Fi' and ID='" ;
          strC = concat(sqlstatment , concat(mac_address , "' and  Cast (( JulianDay('now') - JulianDay(Last_Time_Found) ) * 24 * 60 * 60 As Integer)  < 15*60  ;"));
          #ifdef DEBUG
		  	printf(strC);
		  #endif
       ...
       else
       {
          // no record found
           sqlite3_finalize(selectstmt);
           sqlstatment="";
           sqlstatment = "INSERT INTO Cellphone_Records_15 " \
                   "  values ( " \
                   " 'Wi-Fi' , '" ;

            strC = concat(sqlstatment , mac_address);
            char * strD ;

            strD= concat(strC ,"' , CURRENT_TIMESTAMP ," \
                   "  DATETIME( 'now' ,'05 minutes') , '1' " \
                   " ); " );

			#ifdef DEBUG
            	printf(strD);
			#endif
            result = sqlite3_exec(db, strD, callback, 0, &zErrMsg);
            if (result != SQLITE_OK ) {
	...
	*/
	...
	sqlstatment = "Select * from Records where Cast (( JulianDay('now') - JulianDay(Last_Time_Found) ) * 24 * 60 * 60 As Integer)  < 15*60  and ID ='" ;
    strC = concat(sqlstatment , concat(mac_address , "' and Footprint='"));
	strD = concat(strC , concat(footprint , "' ;"));
	#ifdef DEBUG
    	printf(strD);
	#endif
    ...
    //printf(result);
    struct sqlite3_stmt *selectstmt;
    result = sqlite3_prepare_v2(db, strD, -1, &selectstmt, NULL);
    if (result != SQLITE_OK ) {
    ...
    if (sqlite3_step(selectstmt) == SQLITE_ROW)
       {
          // record found
          sqlite3_finalize(selectstmt);
			sqlstatment = "UPDATE Records set Last_Time_Found= CASE WHEN Cast(( JulianDay('now') - JulianDay(First_Record) ) * 24 * 60 * 60 As Integer)   > 300  THEN CURRENT_TIMESTAMP ELSE Last_Time_Found  END ,  contador=contador+1 WHERE Tecnologia='Wi-Fi' and ID='" ;
			strC = concat(sqlstatment , concat(mac_address , "' and  Footprint='"));
			strD = concat(strC , concat(footprint , "' and  Cast (( JulianDay('now') - JulianDay(Last_Time_Found) ) * 24 * 60 * 60 As Integer)  < 15*60  ;"));
       ...
       else
       {
          // no record found
           sqlite3_finalize(selectstmt);
           sqlstatment="";
           sqlstatment = "INSERT INTO Records " \
                   "  values ( " \
                   " 'Wi-Fi' , '" ;
			...
			strE = concat(strD , footprint);
            char * strF ;

            strF= concat(strE ,"' , CURRENT_TIMESTAMP ," \
                   "  DATETIME( 'now' ,'05 minutes') , '1' " \
                   " ); " );
	...
	sprintf(sql_insert, "INSERT INTO Information_Elements values ( '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',CURRENT_TIMESTAMP,'%s','%s');", mac_address, footprint, IE_utilized, supp_rates, ext_supp_rates, DS_param, HT_cap, ext_cap, VHT_cap, RM_enabled_cap, interworking, supp_operating_classes, vendor_1, vendor_2, vendor_3, vendor_4, received_power, manuf);
...
int main(int argc, char * argv[])
{
	...
	*/
    ...
    char *err_msg = 0;
	#ifdef HAVE_SQLITE
		int rc = sqlite3_open("/home/kali/Desktop/DB/Footprints.db", &db);
